				...
				# Dataset division into two classes 
					if(len(result['masks'])==0): 
						...
						if 2 in result['class_ids']:
							...
							result['blue_masks'] =\
								np.array([v for j, v in enumerate(np.swapaxes(result['masks'], 0, 2)) if (result['class_ids'][j] == 2)])
							result['blue_masks']=np.swapaxes(result['blue_masks'],0,2)
						
							result['blue_rois'] =\
								np.array([v for j, v in enumerate(result['rois']) if result['class_ids'][j] == 2])
							
							result['blue_scores'] =\
								np.array([v for j, v in enumerate(result['scores']) if result['class_ids'][j] == 2])
					
					# Color splash
					splash = color_splash(images[i], result['red_masks'], result['blue_masks'])
